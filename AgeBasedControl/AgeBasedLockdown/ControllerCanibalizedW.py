...
#####################################
    ...
    dSdt =  -(w[:,0] * beta * S * (mtimes(I,mat_old))  +w[:,1] * beta * S * (mtimes(I,mat_school)) +w[:,2] * beta * S * (mtimes(I,matrix4)) )/ repmat(mtimes(tab_N, a), N+1, 1)   +sigma*R
   
    #dSdt = -u * beta * S * ((mtimes(I,a)) / repmat(mtimes(tab_N, a), N+1, 1))  #*(S>=0)*(E>=0)*(I>=0)
    #dSdt[np.isnan(dSdt)] = 0
    dEdt =  (w[:,0] * beta * S * (mtimes(I,mat_old))  +w[:,1] * beta * S * (mtimes(I,mat_school)) +w[:,2] * beta * S * (mtimes(I,matrix4)) )/ repmat(mtimes(tab_N, a), N+1, 1)  - delta * E
    
    dIdt = delta * E - gamma * I 
    
    dRdt = gamma * I   - sigma*R
    #print(sum1(  sum2(w[:,0] *cost_of_lockdown) )*sum2(tab_N[0,0:4]) )
    
    #cost_lockdown = sum1(  sum2(w[:,0] *cost_of_lockdown) )*sum2(tab_N[0,0:4]) +sum1(  sum2(w[:,1] *cost_of_lockdown) )*sum2(tab_N[0,10])
   # cost_lockdown = 
