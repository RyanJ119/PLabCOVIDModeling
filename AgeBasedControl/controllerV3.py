...
def solve_control_problem(problem, max_num_vaccines_per_day, init_S=None,
   ...
   # sigma = 1/60
    ...
    w_max=[1, 1, 1]  ## w_max is the minimum lockdown 
...
#####################################
    ...
    dSdt = ( -1*(1-w[:,2])*(1-w[:,2]) *( ((1-w[:,0]) * beta * S * (mtimes(I,mat_old-mat_only_old)) ) +((1-w[:,0])*(1-w[:,0]) * beta * S * (mtimes(I,mat_only_old)) ) +((1-w[:,1]) *(1-w[:,1]) * beta * S * (mtimes(I,mat_school))) +(beta * S * (mtimes(I,matrix4))) )/ repmat(mtimes(tab_N, a), N+1, 1) )  #+sigma*R
   ...
   # dSdt = ( -1*( ((1-w[:,0]) * beta * S * (mtimes(I,mat_old)) ) +(beta * S * (mtimes(I,mat_school))) +( beta * S * (mtimes(I,matrix4))) )/ repmat(mtimes(tab_N, a), N+1, 1) )  #Elderly Only
    ...
    dEdt = (1-w[:,2])*(1-w[:,2]) *( ((1-w[:,0]) * beta * S * (mtimes(I,mat_old-mat_only_old)) ) +((1-w[:,0])*(1-w[:,0]) * beta * S * (mtimes(I,mat_only_old)) ) +((1-w[:,1]) *(1-w[:,1]) * beta * S * (mtimes(I,mat_school))) +(beta * S * (mtimes(I,matrix4))) )/ repmat(mtimes(tab_N, a), N+1, 1) - delta * E
   ...
   # print(cost_deaths)
    ...
    init_w=np.minimum(np.maximum(pd.DataFrame.to_numpy(pd.read_csv('../Starting point/w.csv', delimiter=',')),0.),np.array([0.6,0.8,1.]))
...
# Now we look for the optimum
   ...
   # upper_bound_gg = vertcat(np.zeros(4 * num_age_groups * N), w_max * np.ones(3*(N+1)))
    ...
    #cost_lockdown=sum2((sum1( mat_school) / sum1(a)) *tab_N )*sum1(cost_of_lockdown_school*sum2(w[:,1]))
    cost_end = sum2(I[N, :] * death_rates)*cost_per_death*90
    cost_smoothness=1/N*sum1(sum2(fabs(w[1:,:]-w[:N,:])))*10**5*5
